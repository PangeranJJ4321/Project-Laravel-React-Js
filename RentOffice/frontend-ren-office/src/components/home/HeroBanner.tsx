        ...
        <section id="Hero-Banner" className="relative flex h-[720px] -mb-[93px]">
          ...
          >
            ...
            <p className="text-lg leading-8 text-[#000929]">
              Kantor yang tepat dapat memberikan impact pekerjaan menjadi lebih baik dan sehat dalam
              tumbuhkan karir.
            </p>
        ...
        <div className="flex flex-col pt-[150px] pb-10 px-[120px] gap-10 bg-[#0D903A]">
          <div className="logo-container flex items-center justify-center flex-wrap max-w-[1130px] h-[38px] mx-auto gap-[60px]">
            {[
              ...
              <img key={index} src={`/assets/images/logos/${logo}`} alt="clients logo" />
            ))}
          </div>
          <div className="flex justify-center gap-[50px]">
            {[
              ...
              <div key={index} className="flex flex-col gap-[2px] text-center">
                <p className="text-xl leading-[30px] text-[#F7F7FD]">{item.label}</p>
                <p className="font-bold text-[38px] leading-[57px] text-white">{item.value}</p>
              </div>
            ))}
          </div>
        </div>
